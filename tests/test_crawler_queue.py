...
async def test_processed_task_updates_lifecycle_and_unblocks_join() -> None:
    ...
    assert queue.active_urls == {task.url}
    ...
    assert queue.processed_urls == {task.url}
    assert queue.get_stats() == {
        "scheduled": 1,
        "queued": 0,
        "active": 0,
...
async def test_failed_task_retains_error_and_updates_stats() -> None:
    ...
    task = await queue.get_next()
    queue.mark_failed(task.url, "Timeout")
    await asyncio.wait_for(queue.join(), timeout=0.1)

    assert queue.failed_urls == {task.url: "Timeout"}
    assert queue.get_stats() == {
        "scheduled": 1,
        "queued": 0,
        "active": 0,
...
@pytest.mark.asyncio
async def test_get_next_waits_until_a_task_is_added() -> None:
    queue = CrawlerQueue()
    waiting_worker = asyncio.create_task(queue.get_next())
    await asyncio.sleep(0)
...
async def test_only_active_tasks_can_be_finished() -> None:
    ...
    with pytest.raises(ValueError, match="not active"):
        queue.mark_failed(task.url, "Second completion")
